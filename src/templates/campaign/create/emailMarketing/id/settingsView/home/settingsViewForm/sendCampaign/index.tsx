...
export const SendCampaign = () => {
  ...
  async function handleValidateSynchronizationCampaign() {
    const response: any = await validateSynchronizationCampaign({ campaignType: "mailing", id: Number(query.id) });
  ...
  async function sendCampaign() {
    const response = await saveAndSendCampaign({ campaignType: "mailing", id: Number(query.id) });
