...
export const SelectSendDateTime = () => {
  ...
  const [sendComingSoon, setSendComingSoon] = useState(campaign?.schedule?.scheduling_type === "as_soon_as_possible");
  ...
  async function handlerSubmit(values: any) {
    ...
    const response: any = sendComingSoon
      ? await updateCampaignSchedule({
          id: Number(campaign?.schedule?.id),
          campaignSchedule: { scheduling_type: 0 },
        })
      : await updateCampaignSchedule({
          id: Number((campaign?.schedule as any).id),
          campaignSchedule,
        });
  ...
  useEffect(() => {
    if (campaign?.schedule) {
      const label =
        campaign.schedule?.scheduling_type === "as_soon_as_possible"
          ? "Enviar lo antes posible"
          : `${moment(campaign?.schedule?.scheduled_at).format("YYYY-MM-DD HH:mm:ss")} - ${
              campaign?.schedule?.scheduling_timezone
            }`;
      setLabel(label);
    }
  }, [campaign]);
